...
var socketEvents = {
    ...
    clients: function (data) {
        if (document.getElementById('socket-output')) {
            document.getElementById('clinet-list').innerHTML = '';

            for (var i = 0; i < data.data.length; i++) {
                var id = data.data[i];
                var template = document.getElementById('btn-template-client').innerHTML;
                document.getElementById('socket-output').innerHTML = `Clinet ${i + 1}: ${id}\n` + document.getElementById('socket-output').innerHTML;
                template = template.replace('{text}', `Client ${i + 1}`);
                template = template.replace('{id}', id);
                document.getElementById('clinet-list').innerHTML += template;
            }
...
function initClientList() {
    document.querySelectorAll('.socket-clinet-send')?.forEach(function (element) {
